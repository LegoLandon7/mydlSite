...
async def login(request: Request):
    """Redirect to Discord for OAuth authorization"""
...
async def callback(request: Request):
    ...
    try:
        ...
        return RedirectResponse(url=f"{os.getenv('FRONTEND_URL', 'http://localhost:5173')}?login=success")
    except Exception as e:
        print(f"Auth error: {e}")
        return RedirectResponse(url=f"{os.getenv('FRONTEND_URL', 'http://localhost:5173')}?login=failed")
